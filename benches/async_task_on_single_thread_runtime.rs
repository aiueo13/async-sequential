...
fn sequential_executor_on_single_thread_runtime(c: &mut Criterion) {
    async_task_with_sequential_executor(c, single_rt(), "sequential_executor_on_single_thread_runtime");
}

fn tokio_spawn_mutex_on_single_thread_runtime(c: &mut Criterion) {
    async_task_with_tokio_spawn_mutex(c, single_rt(), "tokio_spawn_mutex_on_single_thread_runtime");
}
